...
const getUserCart = (req, res) => {
  ...
  const query = `SELECT * FROM cart INNER JOIN products ON products.id = cart.product_id INNER JOIN categories ON categories.id = products.category_id INNER JOIN brands ON brands.id = products.brand_id INNER JOIN users ON users.id=cart.user_id WHERE cart.user_id=? AND cart.is_deleted=0 `;
...
const updateCartQuantity = (req, res) => {
  ...
  const query = `UPDATE cart SET quantityInCart=? WHERE user_id=? AND product_id=? AND is_deleted=0`;
  const data = [quantityInCart, user_id, product_id];
